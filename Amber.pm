...
use vars qw ( $amberhome $leapexec $sanderexec );
...
BEGIN {
  ...
  $leapexec=(exists $ENV{'LEAPEXEC'}) ?
    $ENV{'LEAPEXEC'} : "$amberhome/exe/teLeap";

  $sanderexec=(exists $ENV{'SANDEREXEC'}) ?
    $ENV{'SANDEREXEC'} : "$amberhome/exe/sander";
}
...
sub generateTopCoor {
  ...
  die "cannot find force field input script for $param" 
    if (!-r "$amberhome/dat/leap/cmd/leaprc.$param");
  
  open FFINP,"$amberhome/dat/leap/cmd/leaprc.$param";
  ...
  system "$leapexec -I$amberhome/dat -I$amberhome/dat/leap/lib -I$amberhome/dat/leap/cmd -I$amberhome/dat/leap/parm -f $leapinp > leap.out 2>&1";
...
#  system "$sanderexec $fileopt >& err$$.out";
  system "$sanderexec $fileopt >& /dev/null";
