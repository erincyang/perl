...
## data: handle -> {outlog, cmdlog, enerlog, fromcharmm, tocharmm }
...
sub new {
  ...
  die "Could not find CHARMM executable"
    if ($exec eq "");
  die "No CHARMMDATA" if (! defined $datadir);
  ...
  die "cannot execute binary" if ((!-x $exec) && ($exec !~ "mpirun"));
...
sub setupEnergy {
  ...
  if ($self->{explicitWater}) {
    if (!$self->{_setupPeriodic}) {
...
sub setupFromMolecule {
  ...
  # Charmm can't read "nan" in B-factor field, so set these to zero
  foreach my $c ( @{$mol->activeChains()} ) { 
      if ($#{$c->{atom}}>=0) { 
	  foreach my $a ( @{$c->{atom}} ) {
	      $a->{aux2}=0.0 if (lc($a->{aux2}) eq "nan");
	  }
      }
  }
  ...
  $terminal="blocked"
    if (!defined $terminal && 
	defined $self->{par}->{blocked} && $self->{par}->{blocked});
  ...
  for (my $if=0; $if<=$#{$slist}; $if++) {
    ...
    $segmol->writePDB($fname,translate=>getConvType($self->{par}->{param}),ssbond=>0,compress=>0);
...
#  $self->_sendCommand("ic fill ");
    ...
    if (($self->{par}->{param} eq "19" || $self->{par}->{param} =~ /22/ || 
	 $self->{par}->{param} eq "eef1")) {
      ...
      $self->_sendCommand("hbuild atom cdie eps 80.0 cutnb 8.0 ctofnb 7.5 ctonnb 6.5 shift vshift bygr");
    } else {
...
sub OldreadFromPDB {
  ...
  $mol->writePDB($fname,translate=>getConvType($self->{par}->{param}),compress=>0);
...
sub loadReference {
  ...
  my $fname=hostname."-pdb$$";
  $mol->writePDB($fname,translate=>getConvType($self->{par}->{param}),ssbond=>0,compress=>0);

  $self->_sendCommand("coor set xdir 1.0 dist 0.0 comp select all end");
  $self->_sendCommand("open unit 10 read form name \"$fname\"");
  $self->_sendCommand("read coor pdb comp unit 10 resi");
  $self->_sendCommand("close unit 10");
...
sub initCoordinates {
  ...
  $self->_sendCommand("coor init select $sele end");
}
...
sub setupNonBonded {
  ...
  if ($self->{par}->{cut}) {
    $self->_sendCommand("update atom $self->{par}->{dielec} eps $self->{par}->{epsilon} cutnb $self->{par}->{cutnb} ctofnb $self->{par}->{cutoff} ctonnb $self->{par}->{cuton} shift vshift bygr");
  } else {
    $self->_sendCommand("update atom $self->{par}->{dielec} eps $self->{par}->{epsilon} cutnb 9999.0 ctofnb 9000.0 ctonnb 8000.0 shift vshift bygr");
  }
}
...
sub periodicBoundaries {
  ...
  if (!defined $self->{par}->{boxsize}) {
    ...
    if ($maxx>$maxy && $maxx>$maxz) {
      $self->{par}->{boxsize}=$maxx+1.0;
    } elsif ($maxy>$maxx && $maxy>$maxz) {
      $self->{par}->{boxsize}=$maxy+1.0;
    } else {
      $self->{par}->{boxsize}=$maxz+1.0;
    }
  ...
  $self->_sendCommand("image byseg xcen 0.0 ycen 0.0 zcen 0.0 select .not. resname tip3 end");
  $self->_sendCommand("image byres xcen 0.0 ycen 0.0 zcen 0.0 select resname tip3 end");
  $self->_sendCommand("update bycb cutim $self->{par}->{cutnb} imgfrq -1");
}
...
sub ewald {
  my $self=shift;
  $self->_getpar(@_);
  $self->_sendCommand("update ewald kappa $self->{par}->{pmekappa} pmewald order 4 fftx $self->{par}->{npme} ffty $self->{par}->{npme} fftz $self->{par}->{npme}");
...
sub poissonBoltzmann {
  ...
  my $surface=($self->{par}->{smooth})?"smooth swin 0.3":"watr 1.4 reen";
...
sub getSASAOutput {
  ...
  foreach my $l ( split(/\n/,$self->{_lastOutput}) ) {
    if ($l=~/SURFAC: TOTAL = +([0-9\.]+)/) {
      $sasa->{area}=$1;
      $sasa->{energy}=($1*5.42+920.0)/1000.0;
    }
  }
  return $sasa;
}
