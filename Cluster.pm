...
sub runCluster {
  ...
  my $clustexe=&GenUtil::findExecutable($self->{par}->{method});
  ...
  if (defined $self->{par}->{fraglist}) {
	...
	$options.=" -fitl $self->{par}->{fitfraglist}";
      } else {
	$options.=" -fitxl";
      }
    }
