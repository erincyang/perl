...
use vars qw ( $pi $logfile $scriptloc );
...
sub findExecutable {
  ...
  return "$ENV{MMTSBDIR}/perl/$name" 
    if (defined $ENV{MMTSBDIR} && 
	-d "$ENV{MMTSBDIR}/perl" && 
	-x "$ENV{MMTSBDIR}/perl/$name");
...
#  my $ret=`gzip -t $f |& cat`;
  system("gzip -t $f >& tmp".hostname."$$");
  my $ret = (-s ("tmp".hostname."$$"));
  system("/bin/rm tmp".hostname."$$");
...
#  return 1 if ($ret eq "");
#  return 0;
#  return ($ret!~/not|invalid|violated/);
}
...
## function: bool = exists(filename)
## check whether file exists, with gz support

sub exists {
  my $f=shift;

  return 0 if ($f eq "-");
  return 1 if ((-r $f) || (-r "$f.gz"));
  return 0;

}

## function: bool = zexists(filename)
## check whether file exists and is not empty, with gz support
## note: since empty gzipped files have non-zero size, call them
##       "empty" if the size is less than 100 bytes

sub zexists {
  my $f=shift;

  return 0 if ($f eq "-");
  return 1 if ((-r $f) && ((-s $f) > 99));
  return 1 if ((-r "$f.gz") && ((-s "$f.gz") > 99));
  return 1 if (($f !~ /\.gz$/) && (-r $f) && (! -z $f));
  return 0;

}

## function: remove(filename)
## removes a file or directory if it exists, with gz support
...
## rename a file if it exists, with gz support
...
## link a file if it exists, with gz support
...
## copy a file if it exists, with gz support
...
sub copy {
  ...
  if (-r $f) {
	  ...
	  system("/bin/cp ".$f.".gz ".$g);
      } else {
	  system("/bin/cp ".$f.".gz ".$g.".gz");
      }
  }
...
sub submitRemote {
  ...
  if (!$pid) {
    ...
    if ($subdir) {
      ...
      system "rsh -n $host '( mkdir $dir; cd $dir; mkdir $sdir )'";
      $rdir="$dir/$sdir";
    } else {
      $rdir=$dir;
    }
...
sub readArchiveFile {
  ...
  sysopen(DAD,$file,O_RDWR) || die "cannot open archive file $file";
  sysread(DAD,$header,40);
  $dlen=substr($header,0,10)+0;
  $dcnt=substr($header,10,10)+0;
