...
sub xchangeStep {
  ...
  foreach my $p ( @{$xpairs} ) {
    ...
    for (my $k=0; $k < $self->nUmbrellas(); $k++) {
      if ($self->{biasdata}->[$k]->{type} eq "lambda") {
	&GenUtil::log("ReXServer::xchangeStep",
	...
	$emi = (1.0-$bi)*$emi + $bi*$vi;
        $emj = (1.0-$bi)*$emj + $bi*$vj;
	$eni = (1.0-$bj)*$eni + $bj*$vi;
        $enj = (1.0-$bj)*$enj + $bj*$vj;
      } else {
	&GenUtil::log("ReXServer::xchangeStep",
    ...
    &GenUtil::log("ReXServer::xchangeStep","delta: $delta");
