...
#       printf STDERR "prun nodes: $pnodes, base: $pbase\n";
     }  
      &doJob($mp,$dir,$joblist[$i],$rex,$rexClient,
             $lasttemp,$clogfile,$elogfile,$pnodes,$pbase);
      exit 0;
    } else {
      push(@pidlist,$pid);
    }
...
sub doJob {
  ...
  my $charmm=&CHARMM::new((defined $clogfile)?"$dir/$job/$clogfile":undef,undef,$prunnodes,$prunbase);
