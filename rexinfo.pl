...
if (defined $showclients && $showclients) {
  ...
  for (my $i=1; $i<=$rex->{trun}; $i++) {
    my $cdat=$rex->getClientData($clientid,$i);
    die "cannot get client data record for $clientid"
      if (!defined $cdat);
...
sub showdata {
  ...
  if (defined $files && $files) {
    printf "%s/%s/%s/%s/final.pdb\n",$dir,$cid,lc $rex->getMode($run),&GenUtil::dataDir($rex->getRun($run));
  } else {
    foreach my $item ( @{$sel} ) {
      $item=lc $item;
      ...
      printf "%-6s ",$cid                     if ($item eq "client" || $item eq "clientid");
      printf "%-4d ",$cdat->{cond}->{inx}     if ($item eq "cond" || $item eq "condinx");
